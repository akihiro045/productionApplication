...
{
    public string[] mic = new string[2];
    ...
    void Start()
    {
        cpuAI = Player[1].GetComponent<CpuAI>();
    ...
    void PlayerAction(int i)
    {
        ...
        if(voiceResult[i]==4)
        {
            characterVoiceController[i].playerHp -= 10;
        }
        if (voiceResult[i] == 6)
        {
            if(i==0)
            {
                ...
                Bullet.transform.position += new Vector3(10.0f, 0.0f, 0);
                Bullet.GetComponent<BulletController>().Shoot(new Vector3(10000, 0, 0));
            }
            else if(i==1)
            {
        ...
        if (voiceResult[i] == 7 && characterVoiceController[i].countBomb > 0)
        {
            if(i==0)
            {
                ...
                Bomb.transform.position += new Vector3(10.0f, 0.0f, 0);
                Bomb.GetComponent<BombController>().Throw(new Vector3(3000, 0, 0));
            }
            else if(i==1)
            {
    ...
    void PlayerMoving(int i)
    {
          ...
          characterVoiceController[i].Player.transform.position = Vector3.MoveTowards(characterVoiceController[i].Player.transform.position, characterVoiceController[i].temp, 20.0f * Time.deltaTime);
    }
            ...
            {            
                ...
                else if(loopCount==1)
                {
                    if(delayCount>=150)
                    {
                        ...
                        voiceResult[loopCount] = cpuAI.ActionSelectCpu(characterVoiceController[loopCount].Player.transform.position);
                    }
                    else
                    {
                        delayCount++;
